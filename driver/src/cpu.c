...
static inline void __cpuid_all(u32 leaf, u32 subleaf, u32 *eax,
...
// Find out the number of processor cores
static int cpu_intel_cores_get(struct cpuinfo_x86 *c)
{
...
void cpu_id_get(struct cpuinfo_x86 *cpu_info)
{
    ...
    cpu_info->booted_cores = cpu_intel_cores_get(cpu_info);
...
int cpu_vmx_on(vcpu_t *vcpu)
{
    ...
    if (ret == -1)
    {
        printk(KERN_INFO "[%s] VMXON failed!\n", DEVICE_NAME);
        return -EFAULT;
    }
...
int cpu_vmx_off(vcpu_t *vcpu)
{
    ...
    {
        printk(KERN_INFO "[%s] VMXOFF failed!\n", DEVICE_NAME);
        return -EFAULT;
    }

    // Clears CR4.VMXE if necessary
    if (!vcpu->old_vmxe)
...
int cpu_vm_clear(vcpu_t *vcpu)
{
    ...
    {
        printk(KERN_INFO "[%s] VMCLEAR failed!\n", DEVICE_NAME);
        return -EFAULT;
    }
